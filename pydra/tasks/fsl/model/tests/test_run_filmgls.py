...
def test_FILMGLS(test_data, inputs, outputs):
    ...
    for out_nm in outputs:
        if isinstance(getattr(res.output, out_nm), list):
            assert [os.path.exists(os.path.abspath(x)) for x in getattr(res.output, out_nm)]
        else:
            print(os.path.abspath(getattr(res.output, out_nm)))
            assert os.path.exists(os.path.abspath(getattr(res.output, out_nm)))
